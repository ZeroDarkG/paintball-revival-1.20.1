...
public class ClaymoreBlockEntity extends BlockEntity {
    ...
    private static final float CLAYMORE_DAMAGE = 20.0F;
    ...
    public static void serverTick(Level level, BlockPos pos, BlockState state, ClaymoreBlockEntity be) {
        ...
        // 1. Buscamos al dueño (el jugador que la puso)
        Player owner = be.getOwner();

        // 2. La explosión visual. Ponemos al dueño como "causante"
        level.explode(
                owner, // Antes era null
        ...
        // 3. El daño a los mobs. Usamos al dueño en la fuente de daño
        for (LivingEntity e : living) {
            if (e instanceof Player p && isWearingFullArmorOfColor(p, clayColor)) {
                continue;
            }
            // Importante: explosion(entidad_explotada, dueño)
            // Como el bloque no es una entidad, el primer parámetro es null, el segundo es owner.
            e.hurt(level.damageSources().explosion(null, owner), CLAYMORE_DAMAGE);
        }
    ...
    public @Nullable Player getOwner() {
        if (this.ownerId != null && this.level != null) {
