...
public abstract class BasePelletEntity extends ThrowableItemProjectile {
    ...
    protected void onHitBlock(BlockHitResult result) {
        ...
        // ✅ ACTIVAR DIANA (REDSTONE)
        // Esto hace que el Target Block calcule la señal de redstone (1-15) según la cercanía al centro.
        if (state.is(net.minecraft.world.level.block.Blocks.TARGET)) {
            state.getBlock().onProjectileHit(level(), state, result, this);
        }
    ...
    protected void onHitEntity(EntityHitResult result) {
        ...
        // ✅ REPRODUCIR SONIDO SIEMPRE AL CHOCAR (Antes del filtro de armadura)
        level().playSound(null, getX(), getY(), getZ(),
                SoundEventsRegistry.GUN_SPLAT.get(),
                SoundSource.PLAYERS, 0.9F, 1.0F);

        // ✅ FILTRO DE EQUIPO
        DyeColor pelletColor = getPelletColor();
        if (living instanceof Player player && pelletColor != null) {
            if (isWearingFullArmorOfColor(player, pelletColor)) {
                // Si es el equipo, ya hemos sonado el splat, ahora solo desaparecemos sin dañar
                discard();
                return;
            }
        }

        // --- EL BLINDAJE ANTI-FALLOS ---
        ...
        DamageSource source = level().damageSources().thrown(this, owner instanceof LivingEntity ? (LivingEntity)owner : null);
        ...
        if (living.hurt(source, dmg)) {
            if (owner instanceof ServerPlayer shooter) {
                ...
                // ✅ Aseguramos que si muere, el jugador reciba la XP
    ...
    protected abstract DyeColor getPelletColor();
